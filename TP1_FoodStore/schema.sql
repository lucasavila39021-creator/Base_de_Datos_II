...
*/
-- ============================================================================
-- REGLAS DE NEGOCIO (TRIGGERS) - PARTE 1 TP2
-- ============================================================================

-- Regla 1: Bloquear cambios si el pedido está ENTREGADO o CANCELADO
CREATE OR REPLACE FUNCTION trg_check_estado_pedido()
RETURNS TRIGGER AS $$
BEGIN
    IF OLD.estado IN ('ENTREGADO', 'CANCELADO') THEN
        RAISE EXCEPTION 'No se puede modificar un pedido que ya está %', OLD.estado;
END IF;
RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
-- Regla 2: Validar stock antes de insertar un detalle
...
IF NEW.cantidad > stock_actual THEN
        RAISE EXCEPTION 'Stock insuficiente. Solicitado: %, Disponible: %', NEW.cantidad, stock_actual;
END IF;
RETURN NEW;
END;
$$ LANGUAGE plpgsql;
